package com.ycity.api.demographics;
...
public class DemographicsPageParser extends AbstractPageParser implements PageParser<Demographics> {
    //TODO REMOVE HTML
    private static final String DEMOGRAPHICS_URL = "/patientProfile/profileEdit/profileEdit.html";

    public DemographicsPageParser(DocumentCreator documentCreator) {
    ...
    private String getValueFromControl(Element formElement, String controlName, boolean input) {
        if (input) {
            ...
            Elements select = formElement.select(String.format("select[id=%s] option[selected]",controlName));
            if (!select.isEmpty()) {
