...
public class JagoMessagePageParser extends AbstractPageParser implements PageParser<Message> {
    ...
    private List<Message> processMessageDocument(Serializable id, Elements messageElements,
        ...
        List<Message> messages = new ArrayList<>();
        Set<JagoShowMessageArgs> processed = new HashSet<>();
        for (Element messageElement : messageElements) {
            JagoShowMessageArgs messageArgs = getShowMessageArgsFromElement(messageElement, sent);
            if (messageArgs != null && !processed.contains(messageArgs)) {
                Document messageViewDocument = pageMapper.getPageByParams(messageArgs);
    ...
    private JagoShowMessageArgs getShowMessageArgsFromElement(Element messageElement,
        ...
        if (showMessage.length > 1) {
            ...
            if (matcher.matches() && matcher.groupCount() == 4) {
                return !isSent ?
                    new JagoShowMessageArgs(matcher.group(1), matcher.group(2),
                        removeQuotes(matcher.group(4)), matcher.group(3)) :
                    new JagoShowMessageArgs(matcher.group(1), matcher.group(2), "undefined",
                        removeQuotes(matcher.group(4)), matcher.group(3));
            } else {
